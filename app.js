/* ============================================================
   ...
   Version autonome · Firebase Firestore + Firebase Auth · Vanilla JS
   ============================================================ */
...
// CONSTANTES
// ─────────────────────────────────────────────
// NOTE SÉCURITÉ : il n'y a plus aucun mot de passe codé en dur ici.
// L'accès admin est protégé par Firebase Authentication (voir plus bas)
// et par les règles de sécurité Firestore (fichier firestore.rules fourni).
...
// ─────────────────────────────────────────────
// PAGE : LOGIN (JOUEUR)
// Plus aucun code d'accès requis : seul le nom est nécessaire.
// ─────────────────────────────────────────────
function handleLogoClick() {
  // Astuce d'UI pour révéler la page admin — la vraie protection
  // se fait via Firebase Authentication ci-dessous, pas ici.
  logoClickCount++;
...
function showPage(id) {
  ...
  // La page admin n'a plus besoin d'être "réveillée" manuellement :
  // onAuthStateChanged (plus bas) gère automatiquement l'affichage
  // du panneau ou du mur de connexion selon l'état réel de la session.
}
...
// ─────────────────────────────────────────────
// PAGE : ADMIN — authentification sécurisée
// ─────────────────────────────────────────────
// Remplace l'ancien mot de passe codé en dur par une vraie connexion
// Firebase Authentication. Les comptes admin sont créés une seule fois
// depuis la console Firebase (Authentication > Users) — il n'existe
// aucune page d'inscription publique, donc personne ne peut se créer
// un compte lui-même.
//
// Le rôle admin est en plus vérifié via un document dans la collection
// Firestore `admins/{uid}` : être connecté ne suffit pas, il faut que
// ce document existe. Cela permet aux règles Firestore (voir
// firestore.rules) de vérifier le rôle indépendamment du client.
...
async function adminLogin() {
  ...
  try {
    ...
    // La suite (affichage du panneau) est gérée par onAuthStateChanged.
  } catch (e) {
    err.textContent = 'Échec de connexion : identifiants incorrects.';
  } finally {
...
// Source unique de vérité pour l'accès admin : dès que l'état de
// connexion Firebase change (connexion, déconnexion, rechargement de
// page avec une session déjà active), on vérifie le rôle et on
// affiche la bonne interface.
...
(function init() {
  ...
  // L'affichage réel du panneau admin est géré par onAuthStateChanged
  // ci-dessus : il n'y a plus de drapeau sessionStorage à vérifier ici.
})();
